...
## Drawing circle function -- adapted from https://stackoverflow.com/questions/29732518/draw-a-circle-in-a-bitmap-image-and-crop-pixels-outside-circle-in-r
drawCircle <- function(radius, fire.image) {
  img <- melt(id.var=1:nrow(fire.image), fire.image)
  names(img) <- c("rows","cols","z")
  
  # Find center of image
  center <- c(trunc(median(1:nrow(fire.image))), trunc(median(1:ncol(fire.image))))
  ...
  image(1:nrow(fire.image), 1:ncol(fire.image), matrix(img$z, nrow=nrow(fire.image), byrow=FALSE), col=c("gray80", "green","red"))
}

## Data generation function
data.generate <- function(n,data.type,sigma,fire.length=70){
  ...
  } else if (data.type=="fireSpread") {
    fire.length <- fire.length-1 #so we don't get to the edge of the image
    fire.image <- matrix(130,170,190) #fake fire image
    radius <- dim(fire.image)[1]*fire.curr.time/(2*fire.length) #converting the time into the full length of the fire...
    drawCircle(radius,fire.image)
  }
