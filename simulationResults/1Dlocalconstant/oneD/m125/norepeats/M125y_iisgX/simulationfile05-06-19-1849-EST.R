...
simulation.LCAS <- function(seed,n,data.type="uniformJump",sigma,bw.fixed.value=NULL,
                            repeats=1,ckertype,bwmethod,regtype,lc.bw.factor){
  ...
  bw.lc$bw <- bw.lc$bw/lc.bw.factor  
  model.lc <- npreg(bw.lc)

  ## Set this temporarily...
  model.llc <- model.lc
  if (is.infinite(lc.bw.factor)) {
   	model.llc$mean <- data$Y
  }
  ## and then LCAS for each repeat
    ...
    new.bandwidths <- c(bw.llc$bw[1]/lc.bw.factor,bw.llc$bw[2])
    data <- data.frame(oracle=data$oracle,X=data$X,Y=data$Y,gX=data$Y)
    bw.llc <- npregbw(Y~X+gX,data=data,regtype=regtype,bandwidth.compute=FALSE,bws=new.bandwidths,ckertype=ckertype)
...
data.types <- c("uniformJump","continuous","continuousWithJump")
...
lc.bw.factor <- c(0.2,0.5,1,2,5,Inf)
  ...
  for (sigma in sigma.seq){
    for (lc.factor in lc.bw.factor){
      ...
      write.csv(return.SEs,paste("results/LCASsimulation",date.started,data.type,
                                 sigma,paste0("bwf",1/lc.factor),repeats,"csv",sep = "_"),row.names = FALSE)
    }
  }
}
